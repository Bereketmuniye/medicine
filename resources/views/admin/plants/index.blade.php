                            ...
                            <td class="border-0 py-3">
                                <div class="d-flex align-items-center">
                                    <div class="plant-img me-3 rounded-3 overflow-hidden" style="width: 48px; height: 48px; background: #e8f5e9;">
                                        @if($plant->image)
                                            <img src="{{ asset('storage/' . $plant->image) }}" class="w-100 h-100 object-fit-cover">
                                        @else
                                    ...
                                    <div>
                                        ...
                                        <div class="text-secondary text-xs">{{ $plant->local_name ?? 'No local name' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="border-0 py-3 italic text-secondary">{{ $plant->scientific_name ?? 'Not classified' }}</td>
                            <td class="border-0 py-3 text-dark fw-medium">{{ $plant->region ?? 'Various' }}</td>
                            <td class="border-0 py-3">
                                <span class="badge bg-success-subtle text-success rounded-pill px-3">{{ $plant->uses->count() }} Uses</span>
                            </td>
                            <td class="border-0 py-3 text-end">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-sm rounded-4 p-2">
                                        <li><a class="dropdown-item rounded-3" href="{{ route('admin.plants.edit', $plant) }}"><i class="fa-solid fa-pen-to-square me-2 text-primary"></i> Edit</a></li>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="border-0 py-5 text-center text-secondary">
                                ...
                                <p class="small mb-0">Start building your medicinal plant database.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
